...
1. **Use the cached copy first.** `job_scraper/uk_sponsor_register.csv` holds the last-downloaded register (gitignored — regenerable, never committed). Check its file mtime: this is a large, slow-moving reference document, not a rapidly-changing one, so if the cached copy is more than **30 days** old, re-fetch it rather than trusting a stale copy — otherwise reuse it as-is.
...
| Requires a language you list at a **negligible declared level (A0/A1, "beginner")**, and the posting states an **explicit, hard high bar** for it — a named CEFR level of C1 or above, the word "native," or an explicit-requirement marker ("required," "erforderlich," "fließend," "verhandlungssicher," "muss," "must") | **FAIL — hard stop.** A beginner level cannot plausibly satisfy a stated C1/native/explicit-required bar; treat this the same as an undeclared language, not as a soft gap a recruiter might flex on. Do not score, do not draft. Quote the exact requirement line and the declared level. |
| Requires a language you **do** list at a non-negligible level, but the posting's stated bar (as written — "fluent," "native," "C1+," "business-level") reads as plausibly **higher** than your declared level | **FLAG, then proceed.** Not a fail. Score and draft normally, but surface the gap explicitly in your report to the user (quote both the posting's requirement and your declared level) so they can judge it themselves — bars like "fluent" vary a lot by company and geography, and a recruiter may be flexible. Never silently drop the posting and never silently treat it as a clean pass. |
...
Judge the level comparison the same way you judge everything else in this framework: read both sides as written and reason about it, don't force either into a rigid scale — CEFR letters, LinkedIn-style buckets ("professional working proficiency"), and plain-English words ("conversational," "fluent," "native") all appear in the wild and don't map onto each other precisely. The one exception is the negligible-level row above: a genuinely beginner declared level (A0/A1) against an explicit high bar is unambiguous enough to be a hard FAIL, not a judgment call — this was tightened 2026-09-04 after several German-market spray dispatches burned a full drafting pass on postings requiring native/C1 German against the candidate's declared A0, which the un-tightened gate had only FLAGged. For every other case, when genuinely unsure whether a stated bar exceeds the candidate's level, prefer FLAG over a silent PASS — the human is meant to be the tiebreaker, not the gate.

**Worked example:** a candidate whose Languages table lists Spanish (Native), English (B1/B2), and German (A0). A posting requiring "fluent Russian" → **FAIL**, Russian isn't declared at all. A posting requiring "C1 German" or "Deutsch als Muttersprache" → **FAIL**, German is declared but only at A0 against an explicit high bar — functionally the same gap as an undeclared language. A posting requiring "fluent English" → **FLAG**, English is declared at a non-negligible level (B1/B2) and "fluent" plausibly exceeds it — score and draft the application, but tell the candidate this posting's bar may be a stretch and let them decide. A posting requiring "conversational English" or unspecified English → **PASS**, B1/B2 clears a "conversational" bar cleanly.

## Employment Type Gate — run before scoring

This gate checks whether a posting is actually the kind of role the candidate wants: full-time and permanent. It runs alongside the Eligibility and Language gates above, before scoring, and its verdict is tracked the same way: `/rank` records the result as `employment_type_gate` (PASS/FAIL/FLAG) with a supporting `employment_type_note`, persists both into `seen_jobs.json`, and treats a FAIL as a shortlist veto exactly like a location or language FAIL. `/apply`'s Step 1 re-runs this check rather than trusting the cached triage verdict, the same way it re-verifies the UK Sponsor Register.

Read the posting's employment-type / contract-type wording (Festanstellung/unbefristet vs. Zeitarbeit/ANÜ/befristet/Projekt, "permanent" vs. "contract"/"temporary"/"fixed-term", who's actually hosting the listing) and classify:

| Posting signal | Verdict |
|---|---|
| Explicitly **permanent, direct-employer** ("Festanstellung", "unbefristet", "permanent", "full-time employee") — or silent, with no contrary signal | **PASS.** |
| **Agency/staffing placement** — German ANÜ (Arbeitnehmerüberlassung), "Zeitarbeit", or the listing is hosted via a staffing/recruitment intermediary acting as the employer of record (freelancermap.de, Randstad, Hays, etc., where *they* employ you and place you at a client) | **FAIL — hard stop.** Do not score, do not draft. |
| **Freelance / self-employed** engagement (invoicing as a contractor, no employment relationship at all) | **FAIL — hard stop.** |
| **Direct-employer fixed-term contract** (Elternzeitvertretung/parental-leave cover, maternity/paternity cover, a named project-bounded term, "befristet" from the employer itself rather than a staffing agency) | **FLAG, then proceed.** Not a fail. This is a materially different, often better category than agency placement — it can convert to permanent, and the employment relationship is with the actual employer, not a staffing intermediary. Score and draft normally, but surface the fixed end date and stated reason (parental cover, project, etc.) explicitly in your report so the candidate can decide, the same way a Language Gate FLAG is surfaced rather than silently resolved either way. |

**Why FAIL and FLAG are different severities here:** agency/staffing placement and freelance work are categorically excluded regardless of fit score — the candidate has been explicit that these are out of scope, no exception. A direct-employer fixed-term role is a real gap worth naming, but not one to silently drop or silently draft around; per [[feedback_reviewer_pass_restored]]'s pattern of restoring human-in-the-loop checks, this is exactly the kind of gate where the human is the tiebreaker, not the automation.

**Do not rely on this being caught downstream at `/apply` instead of at `/rank`.** This gate existed only as a memory note (not as part of this framework file) until 2026-09-04, when a `/rank` triage pass scored a 19-month direct-employer fixed-term role (Star Finanz, parental-leave cover) as "Good Fit" and only a much later `/apply` Step 1 agent caught the contract issue — after tokens had already been spent on triage, and only because that particular agent happened to search memory. A fresh-context `/rank` or spray-mode agent has no reason to search memory unless told to; this framework file is what every one of them reads by design, so the check belongs here, not only in a memory file.
