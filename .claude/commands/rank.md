...
- **After a successful fetch, write the Job Posting Cache** - `job_postings/<normalized-url>.json` per the Job Posting Cache section in `.claude/skills/job-application-assistant/04-job-evaluation.md` - with the fetched posting text, so `/apply` can reuse it instead of re-fetching the same URL. Include this instruction in every scoring agent's prompt. Skip the write for jobs marked `expired` - there is no clean posting text to cache.
...
```json
{
  ...
  "employment_type_gate": "PASS" | "FAIL" | "FLAG",
  "employment_type_note": "<quoted contract signal, only when FLAG or FAIL>",
...
`language_gate`/`language_note` come from `04-job-evaluation.md`'s Language Gate — distinct from `language` above, which just records what language the posting is written in. `employment_type_gate`/`employment_type_note` come from that file's Employment Type Gate — run this for every job, not just ones that look suspicious; agency/ANÜ and fixed-term-cover postings don't always announce themselves in the title.
...
5. **Employment type veto:** `employment_type_gate: FAIL` (agency/ANÜ staffing placement, or freelance/self-employed engagement) excludes the job from the shortlist, same as a location FAIL - list it under "Excluded" with the quoted signal from `employment_type_note`. `employment_type_gate: FLAG` (direct-employer fixed-term contract, e.g. parental-leave cover) stays in the ranking with a visible ⏳ marker and `employment_type_note` shown alongside the score - this is a real gap to surface, not a silent pass or a silent drop.
...
- Ranked jobs: set `"status": "ranked"` and add `"rank_score": <overall>`, `"rank_verdict": "<band>"`, `"rank_date": "YYYY-MM-DD"`, `"location_verdict": "PASS"/"FAIL"/"FLAG"` (never the bare `location` key - that is the scraper's place field, e.g. "Aarhus, Denmark", and overwriting it with a verdict destroys the commute-filter data; an entry ranked before this rename may carry a legacy PASS/FAIL/FLAG string in `location` - read that as the verdict when `location_verdict` is absent, and move it to `location_verdict` when re-writing the entry), `"language_gate": "PASS"/"FAIL"/"FLAG"`, `"language_note"` (omit or `null` when `language_gate` is `PASS`), `"employment_type_gate": "PASS"/"FAIL"/"FLAG"`, `"employment_type_note"` (omit or `null` when `employment_type_gate` is `PASS`), plus `"strengths": [...]` and `"gaps": [...]` copied from the scoring agent's Step 2 JSON for that job. These veto fields are as important to persist as the score itself - without them, nothing later (a re-read of `seen_jobs.json`, a debugging session, the user asking "why was this excluded") can recover why a job did or didn't make the shortlist. Entries ranked before `employment_type_gate` existed simply lack it - readers tolerate its absence and never backfill by guessing; re-rank with `--all` to backfill it for real.
...
No separate export step is needed after writing `seen_jobs.json`. The live, filterable, write-back-capable dashboard (`job_scraper/serve_dashboard.js` — `bun run job_scraper/serve_dashboard.js`, then bookmark the URL it prints) reads `seen_jobs.json` fresh on every page load, so nothing here needs to stay in sync with a separate file. (A `seen_jobs.csv` export previously existed for spreadsheet use before the live dashboard existed; it's no longer generated — the dashboard has fully replaced that use case.)
...
- <Title> at <Company> - employment type FAIL: ANÜ/agency staffing placement - [Link](...)
...
- A shortlisted job with `employment_type_gate: FLAG` (a direct-employer fixed-term contract) gets a ⏳ marker next to its Title and its `employment_type_note` (the end date and stated reason, e.g. "parental-leave cover through Apr 2027") quoted in that job's writeup - same treatment, never buried.
...
4. **Deal-breakers veto scores.** A 90-point job that fails a location, language, or employment-type deal-breaker is excluded, not ranked first.
